...
describe('formatSql', () => {
  it('formats a SELECT query using supported clauses', () => {
    expect(
      ...
      '   AND s.single = a.single',
      ' WHERE s.budget - a.rent >= 0',
      'ORDER BY staff_name ASC, budget_surplus DESC',
    ].join('\n'))
  })

  it('formats table creation and insert statements using style-guide blocks', () => {
    expect(formatSql("create table pets (id, name); insert into pets values (1, 'Miso'), (2, 'Nori');")).toBe([
      'CREATE TABLE pets (',
      '    id,',
      '    name',
      ');',
